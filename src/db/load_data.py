...
[ 로드 대상 CSV ]
  ...
  data/raw_ml/food_poi_nationwide.csv     → poi (category=food)
  data/raw_ml/bike_facility_nationwide.csv → poi (category=facility/bike_storage)
  data/raw_ml/culture_poi_nationwide.csv  → poi (category=kculture)
...
# ── 3. food_poi_nationwide.csv → poi ─────────────────────────────────────────────

def _geom_str(lat, lon) -> str | None:
    """(lat, lon) → EWKT POINT 문자열. 유효하지 않으면 None."""
    try:
        la, lo = float(lat), float(lon)
        if 33 < la < 39 and 124 < lo < 132:
            return f"SRID=4326;POINT({lo} {la})"
    except (TypeError, ValueError):
        pass
    return None


def load_food_poi(cur, path: str) -> int:
    if not os.path.exists(path):
        print(f"  ⚠️  파일 없음: {path}")
        return 0

    df = pd.read_csv(path, encoding="utf-8-sig", low_memory=False)
    print(f"  food_poi_nationwide.csv: {len(df):,}행")

    rows = []
    for rec in df.to_dict("records"):
        raw = json.dumps({
            "is_exemplary":         bool(rec.get("is_exemplary", False)),
            "is_tourist_certified": bool(rec.get("is_tourist_certified", False)),
        }, ensure_ascii=False)
        rows.append({
            "name":         str(rec.get("name", ""))[:200],
            "name_en":      None,
            "category":     "food",
            "sub_category": str(rec.get("sub_category", ""))[:50] if rec.get("sub_category") else None,
            "sido":         str(rec.get("sido", ""))[:20] if rec.get("sido") else None,
            "sigungu":      str(rec.get("sigungu", ""))[:30] if rec.get("sigungu") else None,
            "address":      str(rec.get("address", ""))[:300] if rec.get("address") else None,
            "geom":         _geom_str(rec.get("lat"), rec.get("lon")),
            "source":       "soho",
            "image_url":    None,
            "is_24h":       False,
            "raw_data":     raw,
...
# ── 4. bike_facility_nationwide.csv → poi ────────────────────────────────────────

def load_bike_facility_poi(cur, path: str) -> int:
    if not os.path.exists(path):
        print(f"  ⚠️  파일 없음: {path}")
        return 0

    df = pd.read_csv(path, encoding="utf-8-sig", low_memory=False)
    print(f"  bike_facility_nationwide.csv: {len(df):,}행")

    rows = []
    for rec in df.to_dict("records"):
        rows.append({
            "name":         str(rec.get("name", "자전거보관소"))[:200],
            "name_en":      None,
            "category":     "facility",
            "sub_category": "bike_storage",
            "sido":         str(rec.get("sido", ""))[:20] if rec.get("sido") else None,
            "sigungu":      None,
            "address":      str(rec.get("address", ""))[:300] if rec.get("address") else None,
            "geom":         _geom_str(rec.get("lat"), rec.get("lon")),
            ...
            "image_url":    None,
            "is_24h":       False,
            "raw_data":     _sanitize_json(rec.get("raw_data")),
        })
...
# ── 5. culture_poi_nationwide.csv → poi ──────────────────────────────────────────

def load_culture_poi(cur, path: str) -> int:
    if not os.path.exists(path):
        print(f"  ⚠️  파일 없음: {path}")
        return 0

    df = pd.read_csv(path, encoding="utf-8-sig", low_memory=False)
    print(f"  culture_poi_nationwide.csv: {len(df):,}행")
...
# ── 6. road_clean_nationwide.csv → trail ─────────────────────────────────────────
...
def _sanitize_json(v) -> str | None:
    """CSV raw_data 문자열의 NaN → null 치환 후 반환. 비어있으면 None."""
    if not v or (isinstance(v, float) and v != v):  # None / float NaN
        return None
    s = str(v).strip()
    if not s:
        return None
    # Python NaN → JSON null
...
def _upsert_poi(cur, rows: list[dict]) -> None:
    defaults = {
        "name_en": None, "sub_category": None, "sido": None,
        "sigungu": None, "address": None, "image_url": None,
        "is_24h": False, "raw_data": None,
    }
    for r in rows:
    ...
    VALUES (%(name)s, %(name_en)s, %(category)s, %(sub_category)s, %(sido)s, %(sigungu)s,
            %(address)s, ST_GeomFromEWKT(%(geom)s), %(source)s, %(image_url)s,
            %(is_24h)s, %(raw_data)s::jsonb)
    ON CONFLICT DO NOTHING
    """
    sql_without = """
    INSERT INTO poi (name, name_en, category, sub_category, sido, sigungu,
                     address, geom, source, image_url, is_24h, raw_data)
    VALUES (%(name)s, %(name_en)s, %(category)s, %(sub_category)s, %(sido)s, %(sigungu)s,
            %(address)s, NULL, %(source)s, %(image_url)s,
            %(is_24h)s, %(raw_data)s::jsonb)
    ON CONFLICT DO NOTHING
    """
    if rows_with_geom:
...
def main(url: str, target: str | None) -> None:
    ...
    tasks = {
        "poi": [
            ...
            ("food_poi",        lambda: load_food_poi(cur, os.path.join(RAW_DIR, "food_poi_nationwide.csv"))),
            ("bike_facility",   lambda: load_bike_facility_poi(cur, os.path.join(RAW_DIR, "bike_facility_nationwide.csv"))),
            ("culture_poi",     lambda: load_culture_poi(cur, os.path.join(RAW_DIR, "culture_poi_nationwide.csv"))),
        ],
        "trail": [
    ...
    # 단일 poi 하위 타겟 지원 (food_poi / bike_facility / culture_poi)
    POI_SUB_TARGETS = {"food_poi", "bike_facility", "culture_poi"}
    if target in POI_SUB_TARGETS:
        sub_name = target
        tasks = {"poi": [t for t in tasks["poi"] if t[0] == sub_name]}
        target = "poi"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--table", choices=["poi", "trail", "danger", "weather", "all",
                                            "food_poi", "bike_facility", "culture_poi"],
