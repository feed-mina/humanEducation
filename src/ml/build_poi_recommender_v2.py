...
[ MLflow ]
  실험명: kride_poi_recommender
  CLI 파라미터 + Recall@5/10 기록 → 하이퍼파라미터 비교에 활용
...
mlflow.set_experiment("kride_poi_recommender")
mlflow.start_run(run_name=f"cooccurrence_top{args.top_n}_dist{args.max_dist_km}")
mlflow.log_params({
...
mlflow.log_metrics({
    ...
    "baseline_test_recall5":    round(bl_r5, 4),
    "baseline_test_recall10":   round(bl_r10, 4),
    "boost_gain_recall5":       round(test_r5 - test_r5_nb, 4),
})
mlflow.log_artifact(MODEL_PKL)
mlflow.log_artifact(META_JSON)
mlflow.end_run()
