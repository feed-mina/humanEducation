...
[ MLflow ]
  실험명: kride_safety_model
  UI 확인: mlflow ui --port 5000  →  http://localhost:5000
"""
...
# ── MLflow 실험 설정 ───────────────────────────────────────────────────────────
mlflow.set_experiment("kride_safety_model")

# ── RF 하이퍼파라미터 (MLflow log_params 연동용) ──────────────────────────────
RF_N_ESTIMATORS   = 200
RF_MAX_DEPTH      = 10
RF_MIN_SAMPLES    = 3
W_DISTRICT_DANGER = 0.6   # safety_index_v2 = (1-danger)*W + road_attr*(1-W)
W_ROAD_ATTR       = 0.4
...
df_model["safety_index_v2"] = (
    (1 - df_model["district_danger"]) * W_DISTRICT_DANGER
    + df_model["road_attr_score"] * W_ROAD_ATTR
)
...
# ==============================================================================
# STEP 5 & 6: 피처 / 타겟 준비 + 학습  (MLflow run 시작)
# ==============================================================================
print("\n" + "=" * 65)
print("STEP 5+6: 모델 학습 (회귀 + 분류)")
print("=" * 65)

_mlflow_run = mlflow.start_run(run_name="RandomForest_safety")
...
mlflow.log_params({
    ...
    "random_state":      RANDOM_STATE,
    "w_district_danger": W_DISTRICT_DANGER,
    "w_road_attr":       W_ROAD_ATTR,
...
# ── MLflow: 지표 + 아티팩트 기록 후 run 종료 ─────────────────────────────────
mlflow.log_metrics({
    "r2_regressor":  round(r2, 4),
    "mse_regressor": round(mse, 6),
    "f1_classifier": round(f1, 4),
})
# 피처 중요도 (회귀) 개별 기록
for feat, imp in zip(FINAL_FEATURES, rf_reg.feature_importances_):
    mlflow.log_metric(f"feat_imp_reg_{feat}", round(float(imp), 4))

mlflow.sklearn.log_model(rf_reg,  "safety_regressor")
mlflow.sklearn.log_model(rf_cls,  "safety_classifier")
mlflow.log_artifact(SCALER_PATH)
mlflow.log_artifact(DISTRICT_OUT)
mlflow.end_run()
print("  [OK] MLflow run 기록 완료  →  mlflow ui --port 5000")
