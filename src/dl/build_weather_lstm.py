...
[ MLflow ]
  실험명: kride_weather_lstm
  epoch별 train_loss / val_acc 실시간 기록
...
def train(data_dir: str):
    ...
    mlflow.set_experiment("kride_weather_lstm")
...
# [메모]  X_flat 이 구해지는 방법이 잘 이해가 가지 않습니다
    ...
    mlflow.start_run(run_name=f"LSTM_seq{SEQ_LEN}_h{HIDDEN}_l{LAYERS}_e{EPOCHS}")
    mlflow.log_params({
        "seq_len":     SEQ_LEN,
...
# [메모] optimizer.zero_grad()는 왜 해야 하나요?
    ...
    # MLflow: 모델·아티팩트 기록 후 run 종료
    mlflow.pytorch.log_model(model, "weather_lstm")
