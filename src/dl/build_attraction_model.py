...
[ MLflow ]
  실험명: kride_attraction_model
  TabNet 하이퍼파라미터 + MAE/R² 기록
...
def train_tabnet(df: pd.DataFrame):
    ...
    # TabNet 하이퍼파라미터 상수 (MLflow log_params 연동용)
    TABNET_N_D             = 16
    TABNET_N_A             = 16
    TABNET_N_STEPS         = 3
    TABNET_GAMMA           = 1.3
    ...
    mlflow.set_experiment("kride_attraction_model")
    mlflow.start_run(run_name=f"TabNet_nd{TABNET_N_D}_steps{TABNET_N_STEPS}")
    mlflow.log_params({
        "n_d":            TABNET_N_D,
        "n_a":            TABNET_N_A,
        ...
        "lr":             TABNET_LR,
        "weight_decay":   TABNET_WEIGHT_DECAY,
    ...
    model = TabNetRegressor(
        n_d=TABNET_N_D, n_a=TABNET_N_A, n_steps=TABNET_N_STEPS,
        gamma=TABNET_GAMMA, n_independent=TABNET_N_INDEPENDENT, n_shared=TABNET_N_SHARED,
        momentum=0.02, clip_value=2.0,
        optimizer_fn=torch.optim.Adam,
        optimizer_params={"lr": TABNET_LR, "weight_decay": TABNET_WEIGHT_DECAY},
        verbose=10,
        seed=42,
    )
    ...
    mlflow.end_run()
    print("  ✅ MLflow run 기록 완료  →  mlflow ui --port 5000")
