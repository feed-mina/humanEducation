...
| **소비 TabNet** | consume_regressor.zip | MAE=125,302원, R²=0.0053 | R²≥0.25 | **심각 — 즉시 개선 필요** | **1순위** |
...
| **Co-occurrence POI 추천** | poi_cooccurrence.pkl | Recall@5=0.1260 (베이스라인 3.4배) | Recall@5≥0.20 | 양호, 개선 가능 | 3순위 |
...
> **현재 상태**: R²=0.0053 — 사실상 예측 불가 수준. 즉시 개선 필요.
...
| 단계 | 적용 개선 | 예상 R² | 예상 MAE |
|------|---------|---------|---------|
| 현재 | 기준 | 0.0053 | 125,302원 |
| Step A 후 | 이상치 제거 + 로그 변환 | ~0.05 | ~60,000원 |
| Step B 후 | 타겟 재정의 (전체 소비) | ~0.15 | ~45,000원 |
| Step C 후 | 소득/목적/성별 피처 추가 | ~0.25 | ~35,000원 |
| Step D+E 후 | 인코딩 + 계절 보정 | ~0.28 | ~30,000원 |
| Step F 후 | 전국 데이터 (10,000+행) | ~0.35+ | ~25,000원 |
...
| 개선 항목 | 방법 | 예상 효과 |
|---------|------|---------|
| 경기도 레저스포츠 POI 재수집 | 공공데이터포털 타임아웃 재시도 / 다른 시간대 | 83개 → 500개+ 예상 |
| 관광지 반경 확대 | 1km → 2km Spatial Join | tourism_score>0 세그먼트 65% → 80%+ |
| 전국 POI 데이터 추가 | 한국관광공사 TourAPI (전국 관광지 25만 건) | 전국 확대 기반 |

### 4-2. 중기 개선 (Phase 8 SNS 크롤링 연동)

AI 비서의 SNS 크롤링 결과를 ChromaDB에 저장하고, 검색 횟수를 tourism_score에 반영:

```
SNS 크롤링 결과 (장소별 언급 횟수)
        +
기존 tourism_score (POI 밀도)
        +
attraction_score (방문자 만족도)
        ↓
tourism_score_v3 = 0.5×기존 + 0.3×attraction + 0.2×sns_mention_norm
```
...
> **현재**: Recall@5=0.1260, Recall@10=0.1761 (베이스라인 3.4배)
> **목표**: Recall@5≥0.20
...
| 한국관광공사 TourAPI | 일부 | API 쿼터 확대 | 전국 25만 POI |
...
| Co-occurrence | Recall@5=0.126 (2,560 여행) | Recall@5≥0.20 (15,000 여행) |
